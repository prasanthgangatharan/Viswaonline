    ...
    <div className="agent-layout">
      ...
      <div className="agent-main">
        ...
        <div className="agent-mobile-header" style={location.pathname.startsWith('/agent/data-entry') ? { display: 'none' } : undefined}>
          {/* Left: back (if inner page) + username */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 700, color: '#fff', letterSpacing: -0.2 }}>{user?.username}</span>
          </div>

          {/* Right: sign out icon */}
          <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: 10, background: 'rgba(239,68,68,0.25)', border: '1px solid rgba(239,68,68,0.4)', color: '#fff', cursor: 'pointer', flexShrink: 0 }}>
            <LogOut size={16} />
          </button>
        </div>

        {/* Page Content */}
        <main className="agent-content" style={location.pathname.startsWith('/agent/data-entry') ? { padding: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' } : undefined}>
          <Outlet />
        </main>
      </div>
    </div>
